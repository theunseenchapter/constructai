...
# Export
obj_path = r'{self.temp_dir.replace(chr(92), chr(92)+chr(92))}/boq_architectural_{self.scene_id}.obj'
...
print(f"Architectural layout OBJ exported: {{obj_path}}")
print(f"Materials: {{len(bpy.data.materials)}}")
print(f"Objects: {{len(bpy.data.objects)}}")

# Save blend file
blend_path = r'{self.temp_dir.replace(chr(92), chr(92)+chr(92))}/boq_architectural_{self.scene_id}.blend'
bpy.ops.wm.save_as_mainfile(filepath=blend_path)

print("ARCHITECTURAL LAYOUT RENDERING COMPLETE")
'''
        ...
        for extension in ['obj', 'mtl', 'blend']:
            src_file = os.path.join(self.temp_dir, f'boq_architectural_{self.scene_id}.{extension}')
            if os.path.exists(src_file):
                dst_file = os.path.join(output_dir, f'boq_architectural_{self.scene_id}.{extension}')
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print("Usage: python boq_renderer_fixed.py <config_file>")
        sys.exit(1)
